...
void perf_elem_move2tail(list_t *elem, list_t *to_list) {
	cds_list_del(elem);
	CDS_INIT_LIST_HEAD(elem);
	cds_list_add_tail(elem, to_list);
}

void perf_move_progs_to_all(sched_t *sched){
	aff_prog_t	*p, *p_tmp;
	...
	if(!cds_list_empty(spl)){
		cds_list_for_each_entry_safe(p, p_tmp, spl, pl_node) {
			perf_elem_move2tail(&p->pl_node, apl);
		}
	}
}

int perf_get_value(list_t *l){
	aff_prog_t		*prog;
	prog_data_t		*pd;
...
void perf_collect(sched_t *sched, prog_data_t *pd) {
	...
	printf("prog: %s, IPC: %d, Mem BW: %llu\n", pd->prg, pd->ipc_alone, pd->sum_mem_reads);
}

/****************** Termination Management *****************/

void perf_kill_app(sched_t *sched, aff_prog_t *prog)
{
	prog_data_t  	*pd;
	// int 			i;
	pid_t			pid;
...
void perf_kill_all(sched_t *sched){
...
void perf_thaw_sched(sched_t *sched){
	aff_prog_t     	*prog;
	prog_data_t  	*pd;
...
void perf_freeze_sched(sched_t *sched){
	aff_prog_t      *prog, *tmp;
	prog_data_t  	*pd;
	list_t          *pl  = &sched->progs_sch;
	list_t          *apl = &sched->progs_all;
	...
	cds_list_for_each_entry_safe(prog, tmp, pl, pl_node) {
		...
		if (monitoring_phase(sched)) perf_collect(sched, pd);
		else {
			pd->instr_retd_sum += pd->instr_retd;
			sched->target_instr[pd->init_id] -= (long long int) pd->instr_retd;
			if (sched->target_instr[pd->init_id] <= 0) perf_kill_app(sched, prog);
		}
	}

	perf_move_progs_to_all(sched);					// ensure that all progs are in all list
	fprintf(sched->scheduleF,"Updating progress ratio...\n");
	cds_list_for_each_entry(prog, apl, pl_node)	{
		pd = prog->sched_data;
		sched_update_waiting(pd);
		pd->ratio = pd->waiting_time;
		fprintf(sched->scheduleF,"\tPROG: %20s WAITING TIME: %6d CLASS: %3d BW_ALONE: %10.8lf IPC_CO/IPC_ALONE: %10.8lf TARGET INSTR:$ %14lld\n", pd->prg, pd->waiting_time, pd->category, (double) (pd->sum_mem_reads * 64) / 1000000.0, (double) pd->avg_mem_reads / (double) pd->ipc_alone, sched->target_instr[pd->init_id]);
	}
...
static void monitor(sched_t *sched) {
	...
	pd =  prog->sched_data;
	perf_elem_move2tail(&prog->pl_node, &sched->progs_sch);
...
aff_prog_t * perf_max_waiting (sched_t *sched)
{
	aff_prog_t		*prog, *temp, *max;
	prog_data_t  	*pd, *max_pd;
	list_t 			*all = &sched->progs_all;

	if (cds_list_empty(all)) return NULL;

	max = cds_list_entry(all->next, aff_prog_t, pl_node);
	max_pd = max->sched_data;

	cds_list_for_each_entry_safe(prog, temp, all, pl_node)
	{
		pd = prog->sched_data;
		if (pd->waiting_time > max_pd->waiting_time)
		{
			max = prog;
			max_pd = pd;
		}
	...
	cds_list_del(&max->pl_node);
	CDS_INIT_LIST_HEAD(&max->pl_node);

	return max;
}

// Return the program with the minimum fitness function

aff_prog_t * perf_max_fitness (sched_t *sched, list_t *list, long long int bw_rem) {
	aff_prog_t 		*prog, *temp, *min;
	prog_data_t  	*pd, *min_pd;

	if (cds_list_empty(list)) return NULL;

	min = cds_list_entry(list->next, aff_prog_t, pl_node);
	min_pd = min->sched_data;

	fprintf(sched->scheduleF, "Searching for the minimum fitness....\n");

	cds_list_for_each_entry_safe(prog, temp, list, pl_node)
	{
		pd = prog->sched_data;
		pd->fitness = (bw_rem >= pd->sum_mem_reads) ? (bw_rem - pd->sum_mem_reads) : (pd->sum_mem_reads - bw_rem);

		fprintf (sched->scheduleF, "Prog: %20s, with fitness score: %6llu\n", pd->prg, pd->fitness);

		if (pd->fitness < min_pd->fitness)
		{
			min = prog;
			min_pd = pd;
		}
	...
	cds_list_del(&min->pl_node);
	CDS_INIT_LIST_HEAD(&min->pl_node);
	fprintf (sched->scheduleF, "Selecting Prog: %20s, with the MIN fitness score: %6llu\n", min_pd->prg, min_pd->fitness);

	return min;
}

void perf_schedule(sched_t *sched){
	aff_prog_t		*prog;
	prog_data_t  	*pd;
	int				cores_allocated = 0;
	long long int	bw_rem = BW_LIMIT - 291250000;
	list_t			*all = &sched->progs_all;
	...
	if (monitoring_phase(sched)) {
		...
		while (cores_allocated < MAX_CORES) {
			prog = (!cores_allocated) ? perf_max_waiting(sched) : perf_max_fitness(sched, all, bw_rem);
			if (prog != NULL) {
				pd = prog->sched_data;
				bw_rem -= pd->sum_mem_reads;
				if (bw_rem < 0) bw_rem = 0;
				fprintf(sched->scheduleF,"\tPROG: %20s WAITING TIME: %6d CLASS: %3d BW_REM: %10.8lf BW_ALONE: %10.8lf\n", pd->prg, pd->waiting_time, pd->category, (double) (bw_rem * 64) / 1000000.0, (double) (pd->sum_mem_reads * 64) / 1000000.0);
				cds_list_add_tail(&prog->pl_node, sch);
...
void perf_respawn(aff_prog_t *prog){
...
void perf_rebalance(void *arg){
	...
	cds_list_for_each_entry_safe(prog, prog_tmp, &ExecSt.pnew_l, pl_node) 
	{
		...
		else
		{
			...
			sched->progs_nr++;
			pd = prog->sched_data;
			perf_elem_move2tail(&prog->pl_node, &sched->progs_all);
			aff_prog_new_running(prog);
		}
	}
}

void perf_qexpired(void *arg, struct timeval *now)
{
	sched_t 	*sched = arg;
	...
	perf_freeze_sched(sched);			// freeze running programs, update waiting times

#ifdef	TIME_WINDOW
	if ( sched->total_tics < RUN_TICS ) {
#endif	
		perf_schedule(sched);
		perf_thaw_sched(sched);
#ifdef	TIME_WINDOW 
	}else{
		fprintf(stderr, "Terminating...\n");
		ExecSt.massacre 		= 1;
...
REGISTER_SCHEDULER(perf, perf_init, perf_rebalance, perf_qexpired, NULL);
